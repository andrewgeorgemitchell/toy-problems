...
var shuffleDeck = function(deck) {
  let counter = 0;
  let currentIndex = Math.abs(Math.floor(Math.random() * 52 - counter));
  while(counter < 26) {
    const newIndex = Math.abs(Math.floor(Math.random() * 52 - counter)); 
    const newVal = deck[newIndex];
    deck[newIndex] = deck[currentIndex];
    deck[currentIndex] = newVal;
    currentIndex = Math.abs(Math.floor(Math.random() * 52 - counter));
    counter += 1;
  }
  return deck;
};
