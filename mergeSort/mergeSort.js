...
var mergeSort = function(array) {
  // Split the array into mutliple arrays
  let arrOfArrs = array.map(value => [value]);
  // Create merge helper function to handle merges
  const merge = (mergeArr) => {
    // Should grab two arrays and merge them at a time
    const newArr = [];
    for (let i = 0; i < mergeArr.length; i += 2) {
      newArr.push(mergeArr[i].concat(mergeArr[i+1]));
    }
    return newArr;
  }
  arrOfArrs = merge(arrOfArrs);
  // Iterate when final results.length is not equal to N
  while (arrOfArrs.length < array.length) {
    arrOfArrs = merge(arrOfArrs);
  }
  return arrOfArrs;
};
